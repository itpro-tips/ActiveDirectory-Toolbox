...
#>
    ...
    if ($objectsWithStaleAdminAccount) {
        Write-Host -ForegroundColor Yellow "In this forest, found $(($objectsWithStaleAdminAccount | Measure-Object).count) object(s) that are no member of a group protected by AdminSDHolder but still have adminCount attribute = 1. It means the inheritance is disabled on the object(s)."
